...
export class GetUserNotificationUseCase implements UseCase<UserNotificationDTO, Promise<Response>> {
  ...
  async execute(payload: UserNotificationDTO): Promise<Response> {
    try {
      const notification = await this.notificationRepo.getUserNotifications(payload.userId, payload.page, payload.perPage)
